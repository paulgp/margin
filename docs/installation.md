...
Margin 0.4.0 provides one setup command from a trusted checkout:
...
- `dist/margin-cli-0.4.0.tgz`: bundled CLI, package metadata, README, and license; no workspace links, runtime dependencies, or install scripts.
- `dist/margin-0.4.0.vsix`: the bundled VS Code extension.
- `dist/margin-0.4.0.json`: version and SHA-256 checksums for both artifacts.
...
```sh
npm install --global ./margin-cli-0.4.0.tgz --offline --ignore-scripts --no-audit --no-fund
code --install-extension ./margin-0.4.0.vsix --force
margin --version
```
