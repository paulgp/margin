...
const {runCodex,runProcess,codexArgs,verifyRuntime,verifyFeatureSettings,disabledFeatures,supportedVersion,seatbeltProfile} = require('../packages/cli/dist/codex');
const help = '--ignore-user-config --ignore-rules --sandbox --output-schema --output-last-message --ephemeral --strict-config';
...
for (const mode of ['success','version','flags','failure','timeout','overflow','missing','malformed','symlink']) test(`Codex adapter fake executable: ${mode}, with real OS sandbox`, {skip:process.platform!=='darwin'}, async t => {
  ...
  if(mode==='success') {
    const result=await runCodex(request,options);assert.match(result.response,/Fake executable/);assert.equal(result.provenance.reported_model,null);
